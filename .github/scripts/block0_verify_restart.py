...
def assert_results(results: list[CheckResult]) -> None:
    failures = [result for result in results if result.classification != "FUNCIONA VERIFICADO"]
...
def prepare_phase(base_url: str, state_path: Path) -> None:
    ...
    with httpx.Client(base_url=base_url, timeout=30.0) as client:
        ...
        blueprint_id = (
            blueprint_body.get("blueprint", {}).get("blueprint_id")
    ...
    api_event_types = [item["event_type"] for item in final_case_body["events"]]
    ...
    state = {
        ...
        "case_api_status": final_case_body["status"],
        "case_api_event_types": api_event_types,
...
def verify_phase(base_url: str, state_path: Path, output_dir: Path) -> None:
    ...
    with httpx.Client(base_url=base_url, timeout=30.0) as client:
        ...
        recovered_event_types = (
            [item.get("event_type") for item in recovered_body.get("events", [])]
        ...
        post_results.append(
            request_result(
                check_id="P-012",
                name="Case state and public event history survive restart",
                expected_status=200,
                response=recovered,
                semantic_checks=lambda body: [
                    (
                    ...
                    (
                        isinstance(body, dict)
                        and body.get("status") == state["case_api_status"],
                        "case status matches pre-restart state",
                    ),
                    (
                        recovered_event_types == state["case_api_event_types"],
                        "public event types and ordering match pre-restart state",
                    ),
                ],
            )
        )
            ...
            request_result(
                ...
                semantic_checks=lambda body: [
                    ...
                    (
                        ...
                        == "in-memory-development",
                        "loss matches declared in-memory persistence",
                    ),
                ],
            )
        )
    ...
    post_results.append(
        classify(
            check_id="P-014",
            name="Exact Kernel case version and event IDs survive restart",
            expected="Repository case snapshot matches pre-restart snapshot",
            observed=f"version {snapshot['case']['version']}",
            checks=[
                (
                    snapshot["case"] == pre_snapshot["case"],
                    "case id, status, version, event ids and event types match",
                )
            ],
            evidence=snapshot["case"],
        )
    )
    ...
    post_results.append(
        classify(
            ...
            checks=[
                ...
                (
                    counts["events"] == len(pre_snapshot["case"]["event_ids"]),
...
def write_reports(
    ...
    json_path.write_text(
        json.dumps(
            {"state": state, "results": serialize_results(results)},
            ensure_ascii=False,
            indent=2,
        ),
        encoding="utf-8",
    ...
    lines.extend(
        [
            ...
            "- The OAAA blueprint is absent after restart because its control plane remains",
            "  `in-memory-development`.",
...
def main() -> int:
    ...
    try:
        if args.phase == "prepare":
            prepare_phase(args.base_url.rstrip("/"), Path(args.state_file))
        else:
            verify_phase(
